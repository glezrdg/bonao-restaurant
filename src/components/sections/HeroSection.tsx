...
interface HeroSectionProps {
  headline?: string;
  subheadline?: string;
  /** Background image base path (without size suffix, e.g., "/optimized/restaurant/_S8A7285") */
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        <motion.h1
          variants={fadeInUp}
          className="font-garamond text-5xl md:text-7xl lg:text-8xl text-paper mb-6"
        >
          {headline}
        </motion.h1>
        ...
        <motion.p
          variants={staggerItem}
          className="text-lg md:text-xl text-greige max-w-2xl mx-auto mb-10"
        >
          {subheadline}
        </motion.p>
        ...
        <motion.div
          variants={staggerItem}
          className="inline-flex flex-col sm:flex-row items-center gap-4 bg-charcoal/80 backdrop-blur-sm p-4 sm:p-6 rounded-xl border border-line-dark"
        >
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="flex items-center gap-2 text-greige">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <span className="text-sm">Today</span>
            </div>
            <div className="hidden sm:block w-px h-6 bg-line-dark" />
            <div className="flex items-center gap-2 text-greige">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
